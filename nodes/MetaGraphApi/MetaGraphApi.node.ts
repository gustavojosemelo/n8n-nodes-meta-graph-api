...
export class MetaGraphApi implements INodeType {
	description: INodeTypeDescription = {
		...
		description:
			'Interact with Meta Graph API with automatic cursor pagination and batching. Follows "next" cursors automatically — no loops needed.',
		defaults: {
			name: 'Meta Graph API',
		},
		usableAsTool: true,
		...
		properties: [
			// ─── Host URL ───────────────────────────────────
			{
				...
				description:
					'The host URL for the request. Almost all requests use graph.facebook.com. Only video uploads use graph-video.facebook.com.',
				required: true,
			},
			...
			// ─── Node ───────────────────────────────────────
			{
				...
				default: '',
				description:
					'The node to operate on. Each object in Facebook has a unique ID. E.g.: me, act_123456789, page-id.',
			...
			// ─── Edge ───────────────────────────────────────
			{
				...
				default: '',
				description:
					'Edge of the node. Edges are collections of objects attached to the node. E.g.: ads, campaigns, adsets, insights, posts.',
				placeholder: 'ads',
			},
			...
			// ─── Fields ─────────────────────────────────────
			{
				...
				default: '',
				description:
					'Comma-separated list of fields to return. Supports nested fields. E.g.: id,name,effective_status,adcreatives{title,body,thumbnail_url}',
			...
			// ─── Limit ──────────────────────────────────────
			{
				...
				default: 25,
				description:
					'Number of records per page. The Graph API returns up to ~100 per page depending on the endpoint. Use smaller values to avoid "reduce the amount of data" errors.',
				displayOptions: {
					show: {
						httpRequestMethod: ['GET'],
					},
			...
			// ─── Auto Pagination ────────────────────────────
			{
				...
				default: true,
				description:
					'Whether to automatically follow "next" pagination cursors from the Graph API, collecting ALL results without external loops.',
				displayOptions: {
					show: {
						httpRequestMethod: ['GET'],
					},
			...
			// ─── Max Pages ──────────────────────────────────
			{
				...
				default: 0,
				description:
					'Maximum number of pages to fetch. Use 0 for unlimited (fetches everything). Useful to avoid excessive requests on large accounts.',
				displayOptions: {
					show: {
						autoPagination: [true],
						httpRequestMethod: ['GET'],
			...
			},
			{
				...
				description:
					'For Form-Data Multipart, use the format: "sendKey1:binaryProperty1,sendKey2:binaryProperty2"',
			},
			...
			// ─── Options ────────────────────────────────────
			{
				...
				options: [
					...
					},
					{
						...
						description:
							'Additional query string parameters as a JSON object. If set here and above, both are merged.',
					},
					{
						...
						default: 0,
						description:
							'Delay in milliseconds between each pagination page request. Useful to respect API rate limits.',
						typeOptions: {
							minValue: 0,
						},
					},
					{
						...
						default: 500,
						description:
							'Delay in milliseconds between processing each input item. Prevents flooding the API when multiple items are connected (e.g., 31 date ranges). Set to 0 for parallel execution (not recommended for Meta API). Default: 500ms.',
						typeOptions: {
							minValue: 0,
							maxValue: 10000,
						},
					},
					{
						...
						default: 30000,
						description:
							'Timeout in milliseconds for each HTTP request. Increase for slow endpoints or large responses. Default: 30000 (30 seconds).',
						typeOptions: {
							minValue: 5000,
							maxValue: 120000,
						},
						...
						options: [
							{
								...
								value: 'individual',
								description:
									'Each record from the "data" field becomes a separate output item',
							},
							{
								name: 'Raw Response',
								value: 'raw',
								description:
									'Returns the full API response as a single item (including paging info)',
							},
						],
						default: 'individual',
						description:
							'How data should be returned. "Individual Items" is ideal for processing each record separately.',
					},
				],
			},
		],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const buildQueryString = (qs: Record<string, any>): string => {
			const parts: string[] = [];
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				const requestTimeout = (options.requestTimeout as number) || 30000;
				const retryOnConnectionError =
					options.retryOnConnectionError !== undefined
						? (options.retryOnConnectionError as boolean)
						: true;
				const MAX_RETRIES = 3;
				const skipSsl = this.getNodeParameter(
					'allowUnauthorizedCerts',
					itemIndex,
					false,
				) as boolean;
				const itemInterval = (options.itemInterval as number) ?? 500;
				...
				if (options.queryParameters?.parameter) {
				...
				if (options.queryParametersJson) {
					try {
						const jsonParams = JSON.parse(options.queryParametersJson as string);
				...
				let binaryFormData: any = undefined;
				...
				const autoPagination =
					httpRequestMethod === 'GET'
						? (this.getNodeParameter('autoPagination', itemIndex, true) as boolean)
						: false;

				const maxPages = autoPagination
					? (this.getNodeParameter('maxPages', itemIndex, 0) as number)
					: 1;
				...
				while (hasNextPage) {
					...
					try {
						...
						const httpRequestOptions: any = {
							method: httpRequestMethod as 'GET' | 'POST' | 'DELETE',
							url: currentUrl,
							headers: {
						...
						response = await executeWithRetry(
							() => this.helpers.httpRequest(httpRequestOptions),
							MAX_RETRIES,
